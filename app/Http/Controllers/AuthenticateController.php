...
class AuthenticateController extends Controller
{
    ...
    {
        $all   = $request->get('all');
        $stateParams = $request->get('stateParams');
...
//        }
        ...
        if($stateParams['type'] == 'employer'){
            $rules = $employer_rules;
        }else if($stateParams['type'] == 'jobseeker' ){
            $rules = $jobseeker_rules;
        }
...
//        return $validator;
    ...
    public function signup(Request $request)
    {
        ...
        $role = Role::where('name','=',$stateParams['type'])->first();
        $employer_rules = [
            'first_name' => 'required|min:3|regex:/^[a-zA-Z- ]*$/',
            'last_name'  => 'required|min:3|regex:/^[a-zA-Z- ]*$/',
            'email'      => 'email|required|max:255|unique:type_user',
        ...
        if(!Auth::check()){
            ...
            if ($validator->fails()) {
                ...
                $validateObj['password_confirmation'] = 'ok';
