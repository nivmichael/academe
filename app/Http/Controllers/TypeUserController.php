...
//		Mail::send('emails.welcome_jobseeker1',  ['user' => $user], function($message) use ($user)
//		{
//			$message->from('no-reply@site.com', "AcadeME");
//			$message->subject("Welcome to AcadeME");
//			$message->to('dorshoham88@gmail.com');
//		});
		...
		return response()->json([ 'user' => $user,'posts'=> $this->user->getLabeledPosts($request->user()), 'roles'=>$roles ]);
	}
...
//		return response()->json($obj);
		...
		return response()->json([ 'user' => $obj, 'posts' => $this->user->getLabeledPosts($request->user()) ]);
	}
